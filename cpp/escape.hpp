...
template<typename T>
class Base_Stringify {
// public:
//     virtual constexpr const char* c_str() const { return "§"; };
//     virtual constexpr operator const char *() const { return "@"; }
};

template<unsigned... N>
class Stringify : public Base_Stringify<unsigned> {
    constexpr static auto buflen() noexcept {
        unsigned int len = 0;
        auto f = [&len](const vector<unsigned> vec) {
            for (auto v : vec) {
                if (v == 0) len++;
                len++;
    ...
    char buf[buflen()] = {};
public:
    constexpr Stringify() noexcept : Base_Stringify() {
        auto ptr = buf + buflen();
...
// template<typename C> concept Code = requires(C c) { {c} -> std::same_as<unsigned>; };
// template<typename T> concept Code = requires (T t) { {t} -> std::same_as<Stringify>; }


template<int A, int B>
constexpr const char* cstr_concat(const char (&a)[A+1], const char (&b)[B+1]) {
    constexpr int len = A + B + 1;
    char buf[len] = {};
    auto ptr = buf + len;
    *--ptr = '\0';
    for (auto n = B; n; n--)
...
// template<unsigned... U>
// using Foo = Stringify<U...>;

template<Base_Stringify... Ts>
struct codify {};

template<>
struct codify<> : std::integral_constant<char, '\0'> {};

template<Base_Stringify A, Base_Stringify... Z>
struct codify<A, Z...> {};


//template<Base_Stringify S>
//constexpr const char* codify() {
//    return S.c_str();
//}
//template<Base_Stringify S="", Base_Stringify... X>
//constexpr const char* codify() {
//    return cstr_concat(S.c_str(), codify<X...>());
//}

class Ansi {
public:
    template<Base_Stringify... codes> static constexpr string ansify(const char*);

    static constexpr Stringify<0>     RESET                      ;
    static constexpr Stringify<1>     BOLD                       ;
    ...
    static constexpr Stringify<31>    FG_RED                     ;
    ...
    struct FG {
        template<unsigned color> static constexpr Stringify<38, 5, color> std;
        template<unsigned r, unsigned g, unsigned b> static constexpr Stringify<38, 2, r, g, b> rgb;
    };

    struct BG {
        template<unsigned color> static constexpr Stringify<48, 5, color> std;
        template<unsigned r, unsigned g, unsigned b> static constexpr Stringify<48, 2, r, g, b> rgb;
    };
};
